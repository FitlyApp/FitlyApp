document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!dados) {
    ...
    document.querySelector(".etapa").textContent =
      "Dados não encontrados. Por favor, preencha o formulário novamente.";
    return;
  }
  ...
  const carboidratos = (caloriasSemana * 0.4) / 4;
  const proteinas = (caloriasSemana * 0.3) / 4;
  const gorduras = (caloriasSemana * 0.3) / 9;

  document.getElementById("resultado-calorias").textContent =
    Math.round(caloriasSemana);
  document.getElementById("resultado-fds").textContent =
    Math.round(caloriasFds);
  document.getElementById("resultado-proteinas").textContent =
    Math.round(proteinas) + "g";
  document.getElementById("resultado-carboidratos").textContent =
    Math.round(carboidratos) + "g";
  document.getElementById("resultado-gorduras").textContent =
    Math.round(gorduras) + "g";
  ...
  if (condicao === "sim") {
    disclaimerP.innerHTML =
      "<strong>Atenção:</strong> Como você indicou possuir uma condição de saúde, os valores calculados são apenas uma estimativa. É <strong>essencial</strong> que procure um médico ou nutricionista antes de iniciar qualquer plano.";
    disclaimerP.style.color = "#d9534f";
  } else {
    disclaimerP.textContent =
      "Atenção: Este é um valor estimado. Para um plano alimentar preciso, consulte um nutricionista.";
  }
});
